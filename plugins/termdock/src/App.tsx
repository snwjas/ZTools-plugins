...
const THEME_PREFERENCE_MIGRATION_KEY = 'termdock.theme.preference.v2'
const LOCALE_STORAGE_KEY = 'termdock.locale'
const MAIN_TAB_UI_STORAGE_KEY = 'termdock.main.tab-ui'
const LAYOUT_STORAGE_KEY = 'termdock.layout.v1'
const TERMINAL_FONT_SIZE_STORAGE_KEY = 'termdock.terminal.fontSize'
const DEFAULT_SIDEBAR_WIDTH = 214
const DEFAULT_FILE_PANEL_HEIGHT = 218
const DEFAULT_TERMINAL_FONT_SIZE = 13
const TERMINAL_FONT_SIZE_OPTIONS = [11, 12, 13, 14, 16]
...
function readStoredLayoutState(): StoredLayoutState {
  if (typeof window === 'undefined') {
    return {
      ...
      filePanelHeight: DEFAULT_FILE_PANEL_HEIGHT,
      hasStoredFilePanelHeight: false
    }
  }
  ...
  try {
    ...
    const hasStoredFilePanelHeight = parsed.hasStoredFilePanelHeight === true
      && typeof parsed.filePanelHeight === 'number'
      && Number.isFinite(parsed.filePanelHeight)
    return {
      sidebarWidth: clampNumber(parsed.sidebarWidth, MIN_SIDEBAR_WIDTH, MAX_SIDEBAR_WIDTH, DEFAULT_SIDEBAR_WIDTH),
      filePanelHeight: clampNumber(parsed.filePanelHeight, 25, 900, DEFAULT_FILE_PANEL_HEIGHT),
...
function writeStoredLayoutState(state: StoredLayoutState) {
  if (typeof window === 'undefined') {
    return
  }
  ...
  try {
    window.localStorage.setItem(LAYOUT_STORAGE_KEY, JSON.stringify({
      sidebarWidth: clampNumber(state.sidebarWidth, MIN_SIDEBAR_WIDTH, MAX_SIDEBAR_WIDTH, DEFAULT_SIDEBAR_WIDTH),
      filePanelHeight: clampNumber(state.filePanelHeight, 25, 900, DEFAULT_FILE_PANEL_HEIGHT),
      hasStoredFilePanelHeight: state.hasStoredFilePanelHeight
    }))
  } catch {
    // Ignore storage failures; layout can fall back to defaults.
  }
}

function normalizeTerminalFontSize(value: unknown) {
  const numericValue = typeof value === 'number' ? value : Number(value)
  return TERMINAL_FONT_SIZE_OPTIONS.includes(numericValue)
    ? numericValue
...
function readStoredTerminalFontSize() {
  ...
  try {
    return normalizeTerminalFontSize(window.localStorage.getItem(TERMINAL_FONT_SIZE_STORAGE_KEY))
  } catch {
    return DEFAULT_TERMINAL_FONT_SIZE
  }
}
...
function writeStoredMainTabUiState(enabled: boolean, state: StoredMainTabUiState) {
  ...
  try {
    const localTabs = state.localTabs.filter((tab) => tab.kind === 'home' || tab.kind === 'system')
    const localTabIds = new Set(localTabs.map((tab) => tab.id))
    const activeLocalTabId = state.activeLocalTabId && localTabIds.has(state.activeLocalTabId)
      ? state.activeLocalTabId
      : null
    const tabOrder = state.tabOrder.filter((key) => !key.startsWith('home:') || localTabIds.has(key.slice(5)))
    window.localStorage.setItem(MAIN_TAB_UI_STORAGE_KEY, JSON.stringify({
      ...state,
      localTabs,
      activeLocalTabId,
      tabOrder
...
function readInitialTheme(searchParams: URLSearchParams): ThemeMode {
  ...
  if (queryTheme === 'system' || queryTheme === 'default-light' || queryTheme === 'default-dark') {
    return queryTheme
  }
  ...
  try {
    const hasMigratedThemePreference = window.localStorage.getItem(THEME_PREFERENCE_MIGRATION_KEY) === '1'
    const nextTheme = window.localStorage.getItem(THEME_STORAGE_KEY)
    if (!hasMigratedThemePreference && nextTheme === 'default-dark') {
      return 'system'
    }
    return nextTheme === 'system' || nextTheme === 'default-light' || nextTheme === 'default-dark' ? nextTheme : 'system'
  } catch {
    return 'system'
  }
}
...
function isSameFileEditorTab(tab: LocalTab, file: FileContentSnapshot) {
  return tab.kind === 'file-editor'
    && tab.file.source === file.source
    && tab.file.path === file.path
    && (tab.file.tabId ?? '') === (file.tabId ?? '')
}

function uniqueTabOrderKeys(keys: string[]) {
...
function replaceTabOrderKey(keys: string[], oldKey: string, newKey: string) {
  const next = uniqueTabOrderKeys(keys.map((key) => key === oldKey ? newKey : key))
  return next.includes(newKey) ? next : [...next, newKey]
}
...
export function App() {
  ...
  const storedLayoutStateRef = useRef<StoredLayoutState | null>(null)
  if (storedLayoutStateRef.current === null) {
    storedLayoutStateRef.current = readStoredLayoutState()
  }
  const [sidebarWidth, setSidebarWidth] = useState(() => storedLayoutStateRef.current?.sidebarWidth ?? DEFAULT_SIDEBAR_WIDTH)
  const [filePanelHeight, setFilePanelHeight] = useState(() => storedLayoutStateRef.current?.filePanelHeight ?? DEFAULT_FILE_PANEL_HEIGHT)
  const [hasStoredFilePanelHeight, setHasStoredFilePanelHeight] = useState(() => Boolean(storedLayoutStateRef.current?.hasStoredFilePanelHeight))
  ...
  const resolvedThemeMode = useThemeMode(themeMode)
  ...
  const useNativeChildWindows = Boolean(desktopApi && !('isZToolsPlugin' in desktopApi))
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(THEME_PREFERENCE_MIGRATION_KEY, '1')
  ...
  useEffect(() => {
    try {
      ...
      // Ignore storage failures; terminal font size can fall back to defaults.
    }
  }, [terminalFontSize])
  ...
  useEffect(() => {
    const onStorage = (event: StorageEvent) => {
      if (event.key === THEME_STORAGE_KEY) {
        ...
        if (nextTheme === 'system' || nextTheme === 'default-dark' || nextTheme === 'default-light') {
  ...
  useEffect(() => {
    ...
    setTabOrder((prev) => {
      const kept = uniqueTabOrderKeys(prev.filter((key) => allKeys.includes(key)))
      ...
      if (replacementKey && missing.length) {
        ...
        if (replaceIndex !== -1) {
          ...
          const nextOrder = uniqueTabOrderKeys([...next, ...missing.slice(1)])
          return areStringArraysEqual(prev, nextOrder) ? prev : nextOrder
        }
      }

      const nextOrder = uniqueTabOrderKeys([...kept, ...missing])
      return areStringArraysEqual(prev, nextOrder) ? prev : nextOrder
    })
  ...
  useEffect(() => {
    ...
    const nextLocalTabs = localTabs.filter((tab) => {
      if (tab.kind === 'home') {
        return true
      }
      if (tab.kind === 'system') {
        return validSessionTabIds.has(tab.sessionTabId)
      }
      return tab.file.source === 'local'
        || !tab.file.tabId
        || validSessionTabIds.has(tab.file.tabId)
    })
  ...
  }, [filePanelHeight, hasStoredFilePanelHeight, sidebarWidth])

  const handleFilePanelHeightChange = (height: number, userAdjusted = false) => {
    setFilePanelHeight(height)
    if (userAdjusted) {
      setHasStoredFilePanelHeight(true)
    }
  }
  ...
  const applySnapshot = (snapshot: WorkspaceSnapshot) => {
    setWorkspace({
      ...
      commandFolders: [...(snapshot.commandFolders || [])],
  ...
  const applyWorkspaceAction = (action?: Promise<WorkspaceSnapshot>) => {
    if (!action) {
      return
    }
    void action
      ...
      .catch((err: Error) => {
        reportError(setError, '更新工作区', err)
      })
  }
  ...
  const openCreateConnection = () => {
    if (useNativeChildWindows && desktopApi) {
  ...
  const handleSaveProfile = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const latestSnapshot = await desktopApi.getSnapshot().catch(() => snapshot)
  ...
  const handleOpenProfile = async (profileId: string) => {
    ...
    try {
      ...
      if (activeHomeId && snapshot.activeTabId && replacementKey) {
        ...
        setTabOrder((prev) => replaceTabOrderKey(prev, replacementKey, nextSessionKey))
  ...
  const closeSessionTabById = async (tabId: string) => {
    ...
    const relatedLocalTabs = localTabsRef.current
      .filter((tab) => (
        (tab.kind === 'system' && tab.sessionTabId === tabId)
        || (tab.kind === 'file-editor' && tab.file.source === 'remote' && tab.file.tabId === tabId)
      ))
      .map((tab) => tab.id)
  ...
  const handleCloseFileEditorTab = (tabId: string) => {
    setLocalTabs((prev) => {
      ...
      if (remaining.length === 0 && workspace.tabs.length === 0) {
        ...
        setNextHomeTabNumber(2)
        setTabOrder((prevOrder) => {
          const filtered = prevOrder.filter((key) => key !== homeTabKey(tabId))
          return filtered.includes('home:home-1') ? filtered : ['home:home-1', ...filtered]
        })
        return [{ id: 'home-1', kind: 'home', title: t.untitledTab }]
      }
      ...
      setTabOrder((prevOrder) => prevOrder.filter((key) => key !== homeTabKey(tabId)))
      return remaining
    })
  }
  ...
  const handleTabContextAction = async (
    action: 'copyName' | 'copySession' | 'connect' | 'connectAll' | 'disconnect' | 'close' | 'closeOthers' | 'closeAll'
  ) => {
    if (!tabContextMenu) {
      return
    }
    ...
    if (action === 'copySession') {
      ...
      const sourceSession = workspace.sessions[target.id]
      if (!sourceTab || !sourceSession) {
        return
      }
      ...
      try {
        ...
        if (snapshot.activeTabId) {
          setTabOrder((prev) => insertTabKeyAfter(prev, sessionTabKey(snapshot.activeTabId!), sessionTabKey(target.id)))
        }
        setActiveLocalTabId(null)
        setError(null)
      } catch (err) {
        reportError(setError, '复制会话', err)
      } finally {
        setIsBusy(false)
      }
      return
  ...
  const openFileEditorInTab = (file: FileContentSnapshot) => {
    const existingTab = localTabsRef.current.find((tab) => isSameFileEditorTab(tab, file))

    if (existingTab) {
      setLocalTabs((prev) => prev.map((tab) => tab.id === existingTab.id ? { ...tab, title: file.name, file } : tab))
      setActiveLocalTabId(existingTab.id)
      setFileEditorError(null)
      return
    }

    const nextId = `file-editor-${globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`}`
    const activeOrderKey = activeLocalTabId
      ? homeTabKey(activeLocalTabId)
      : activeTab
        ? sessionTabKey(activeTab.id)
        : null

    setLocalTabs((prev) => [...prev, { id: nextId, kind: 'file-editor', title: file.name, file }])
    setTabOrder((prev) => insertTabKeyAfter(prev, homeTabKey(nextId), activeOrderKey))
    setActiveLocalTabId(nextId)
    setFileEditorError(null)
  }
  ...
  const handleOpenLocalItem = async (item: LocalFileItem) => {
    ...
    try {
      ...
      const content = await desktopApi.readLocalFile(item.path, 'utf-8')
      openFileEditorInTab({
        source: 'local',
        path: item.path,
        name: item.name,
  ...
  const openRemoteFileForEdit = async (tabId: string, item: RemoteFileItem) => {
    ...
    const content = await desktopApi.readRemoteFile(tabId, item.path, 'utf-8')
    openFileEditorInTab({
      source: 'remote',
      path: item.path,
      name: item.name,
  ...
  const updateFileEditorTab = (tabId: string, file: FileContentSnapshot) => {
    setLocalTabs((prev) => prev.map((tab) => (
      tab.id === tabId && tab.kind === 'file-editor'
        ? { ...tab, title: file.name, file }
        : tab
    )))
  }

  const getFileEditorTabFile = (tabId: string) => {
    const tab = localTabsRef.current.find((entry) => entry.id === tabId)
    return tab?.kind === 'file-editor' ? tab.file : null
  }

  const handleSaveFileEditorTab = async (tabId: string, content: string, encoding: string) => {
    const file = getFileEditorTabFile(tabId)
    if (!desktopApi || !file) {
      return
    }
    ...
    try {
      ...
      if (file.source === 'local') {
        ...
        const targetTabId = file.tabId ?? activeTab?.id
        if (!targetTabId) {
          throw new Error(locale === 'zhCN' ? '没有可用的远程会话' : 'No active remote session')
        }
        const snapshot = await desktopApi.writeRemoteFile(targetTabId, file.path, content, encoding)
        applySnapshot(snapshot)
      }
      updateFileEditorTab(tabId, { ...file, content, encoding })
      setFileEditorError(null)
    } catch (err) {
      reportError(setFileEditorError, '保存文件', err, { targetPath: file.path })
    } finally {
      setIsBusy(false)
    }
  }

  const handleReloadFileEditorTabWithEncoding = async (tabId: string, encoding: string) => {
    const file = getFileEditorTabFile(tabId)
    if (!desktopApi || !file) {
      return
    }
    ...
    try {
      setIsBusy(true)
      const content = file.source === 'local'
        ? await desktopApi.readLocalFile(file.path, encoding)
        : (file.tabId ?? activeTab?.id)
          ? await desktopApi.readRemoteFile(file.tabId ?? activeTab!.id, file.path, encoding)
          : file.content
      updateFileEditorTab(tabId, { ...file, content, encoding })
      setFileEditorError(null)
    } catch (err) {
    ...
    return (
      <>
        <ConnectionManagerModal
          ...
          onCreateFolder={(name) => applyWorkspaceAction(desktopApi?.createFolder(name))}
          onDeleteFolder={(id) => applyWorkspaceAction(desktopApi?.deleteFolder(id))}
          onUpdateFolder={(id, updates) => applyWorkspaceAction(desktopApi?.updateFolder(id, updates))}
          onUpdateOrder={(id, parentId, order) => applyWorkspaceAction(desktopApi?.updateEntityOrder(id, parentId, order))}
        />
        {showForm ? (
          <ConnectionModal
  ...
  if (isFileEditorWindow) {
    return (
      <div className="standalone-shell file-editor-window">
        <div className={`modal-card file-editor-modal ${resolvedThemeMode === 'default-dark' ? 'file-editor-modal--dark' : ''} standalone`}>
    ...
    <>
      <div className="fs-shell" style={{ '--sidebar-width': `${sidebarWidth}px` } as CSSProperties}>
        <TabBar
          ...
          onSetTerminalFontSize={(fontSize) => setTerminalFontSize(normalizeTerminalFontSize(fontSize))}
        ...
        <main className={`fs-main ${error ? 'has-status' : 'no-status'} ${showSidebar ? '' : 'full-width'}`}>
          ...
          <div className="workspace-stage">
            <WorkspaceStage
              ...
              fileEditorError={fileEditorError}
              filePanelHeight={filePanelHeight}
              hasStoredFilePanelHeight={hasStoredFilePanelHeight}
              terminalFontSize={terminalFontSize}
              themeMode={resolvedThemeMode}
              onCloseFileEditorTab={handleCloseFileEditorTab}
              onFilePanelHeightChange={handleFilePanelHeightChange}
              ...
              onReloadFileEditorTab={(tabId, encoding) => {
                void handleReloadFileEditorTabWithEncoding(tabId, encoding)
              }}
              ...
              onSaveFileEditorTab={(tabId, content, encoding) => {
                void handleSaveFileEditorTab(tabId, content, encoding)
              }}
...
function extractDroppedLocalPaths(event: DragEvent<HTMLDivElement>) {
  ...
  const apiFilePaths = desktopApi?.getDroppedFilePaths?.(fileList) ?? []
  const filePaths = apiFilePaths.length
    ? apiFilePaths.filter((filePath): filePath is string => Boolean(filePath))
    : fileList.map((file) => (file as File & { path?: string }).path).filter((filePath): filePath is string => Boolean(filePath))
