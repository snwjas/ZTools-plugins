...
export type OrderedTabEntry =
  | { key: string; kind: 'local'; id: string; title: string; tabKind: 'home' | 'system' | 'file-editor' }
...
export function TabBar({
  ...
  const terminalFontSizeItems = [
    { label: t.terminalFontSizeSmall, value: 11 },
    { label: t.terminalFontSizeCompact, value: 12 },
    { label: t.terminalFontSizeDefault, value: 13 },
    { label: t.terminalFontSizeLarge, value: 14 },
    { label: t.terminalFontSizeHuge, value: 16 }
  ]
      ...
      <div className="titlebar-tabarea">
            ...
            entry.kind === 'local' ? (
              <div
                ...
                className={`fs-tab ${
                  entry.tabKind === 'home'
                    ? 'home-tab'
                    : entry.tabKind === 'file-editor'
                      ? 'file-tab'
                      : 'system-tab'
                } ${activeHomeTabId === entry.id ? 'active' : ''}`}
                draggable
        ...
        {toolsMenu ? (
          <ContextMenu
            ...
            items={[
              { label: `${t.theme}: ${t.followSystem}`, disabled: theme === 'system', action: () => onSetTheme('system') },
              ...
              { separator: true },
              ...terminalFontSizeItems.map((item) => ({
                label: `${t.terminalFontSize}: ${item.label}`,
                disabled: terminalFontSize === item.value,
                action: () => onSetTerminalFontSize(item.value)
              })),
              { separator: true },
