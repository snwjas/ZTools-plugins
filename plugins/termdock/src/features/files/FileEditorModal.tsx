...
export function FileEditorModal({
  ...
  const frameClassName = `modal-card file-editor-modal ${themeMode === 'default-dark' ? 'file-editor-modal--dark' : ''} ${standalone ? 'standalone' : ''} ${embedded ? 'embedded' : ''}`
  const contentNode = (
  ...
  if (embedded) {
    return <div className="file-editor-embedded">{contentNode}</div>
  }
