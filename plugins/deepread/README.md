...
- 支持填写阅读 APP 常见 JSON 书源，搜索在线书籍并导入书架阅读
- 书架按本地书籍和在线书籍分区展示
- 书架、阅读进度保存、分页阅读、目录解析、全文搜索跳转
- 字号、行距、底栏尺寸、透明度、主题、自动翻页设置
- 低调外观、快速隐藏、鼠标移出隐藏、滚轮/键盘翻页
- ZTools 指令支持：打开、导入、搜书、上一页、下一页、隐藏/显示、关闭、自动翻页、低调模式
...
在“找书”面板粘贴阅读 APP 常见 JSON 书源，可为单个书源对象或书源数组。当前支持常用的 `bookSourceName`、`bookSourceUrl`、`searchUrl`、`ruleSearch`、`ruleBookInfo`、`ruleToc`、`ruleContent` 字段，以及常见 CSS 选择器、`class.xxx` / `tag.xxx`、`@text` / `@href` / `@html` 和简单 JSON 字段路径。

搜索到书籍后点击导入，DeepRead 会抓取目录和章节正文并缓存为在线书籍，之后可像本地书一样分页阅读、保存进度、使用目录和底栏阅读窗口。
